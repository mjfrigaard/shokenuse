---
title: "Using the Anthropic Admin API"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using the Anthropic Admin API}
...
The **Admin API** utilities in shokenuse (`util_fetch_usage()` and
`util_fetch_cost()`) provide programmatic access to your organization's
historical token usage and cost data from Anthropic's reporting endpoints.

> **Important:** These functions require an **Admin API key**
> (`sk-ant-admin...`) provisioned through the Anthropic Console. They are
> **not available for individual (personal) accounts** — only for
...
`util_fetch_usage()` wraps the
`GET /v1/organizations/usage_report/messages` endpoint. It handles
pagination automatically and returns a tidy tibble.
...
`util_fetch_cost()` wraps `GET /v1/organizations/cost_report`. Costs are
daily only (`bucket_width = "1d"` is fixed).
...
Both functions handle pagination internally (via `max_pages`, default 10).
For very large date ranges you may need to increase this:
...
The structure below mirrors what `util_fetch_usage()` returns, so you
can prototype visualizations without credentials.
...
```{r merge-programmatic}
api_usage <- util_fetch_usage(
  starting_at = Sys.time() - 30 * 24 * 3600,
  group_by    = "model"
)

# Convert to usage schema
api_rows <- api_usage |>
  dplyr::mutate(
    timestamp             = as.POSIXct(timestamp_bucket, tz = "UTC"),
    machine               = "api",
...
# Combine with local Claude Code data
local_usage <- util_read_claude_code()
all_usage   <- util_combine_usage(local_usage, api_rows)

launch_app()
```

### Interactively (via the dashboard)

Open the **API Data** tab in the dashboard, enter your Admin API key,
configure the date range and grouping, click **Fetch Usage**, then check
**Include in dashboard filters** and click **Merge into dashboard**.
