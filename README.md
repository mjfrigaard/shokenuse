...
A Shiny dashboard for tracking Anthropic API token usage and estimated costs across models, projects, and sessions.
...
**1. Admin API key** — fetches the last 30 days from your Anthropic organization account.  
Enter your `sk-ant-admin...` key, give the source a Machine label (e.g. `"org-name"`), click **Fetch Usage**, then **Add to Dashboard**. Repeat with a different key to load data from another organization.

**2. Upload CSV** — paste in a usage export from the Anthropic Console or another tool.  
Download an R or shell conversion script from the Data tab to export your local `~/.claude/projects/` session logs first.
...
The `util_*` functions work independently of the dashboard for scripted analysis:
...
See `vignette("api-usage")` for the full Admin API reference.
